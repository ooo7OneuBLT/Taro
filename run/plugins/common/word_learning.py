...
【出す指標】
  ...
  注意：ここには「lexicon.assoc() の累積平均とは独立した目安になる」と書いて
  ありましたが、**その lexicon は2026-09-15に削除済み**です。いまは比較相手が無く、
  このEMAだけが「正解物の素の見え方」の目安です（分離度 assoc_sep は常に None）
...
#   「（当時あった lexicon.assoc() の累積平均とは）独立に動く目安を作る」ことが本質。
#   lexicon は2026-09-15に削除したので、いまは比較相手そのものが無い。
_EMA_ALPHA = 0.2
