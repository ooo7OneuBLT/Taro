...
# ---------------------------------------------------------------- 機構の一覧
# (id, 日本語, 英語, 列, 根拠, 説明, ONの判定, primary_axis, deviation_keywords)
#   ONの判定は Config を受け取って True/False を返す関数。None なら常にON。
#
# 【なぜ2フィールド増やしたか、2026-08-15】常駐アプリ（TaroMap）が「生得／創発／道具」の
#   区分と「逸脱リストとの対応」を見せたいという依頼のため。
#   primary_axis   None（未確定）| "innate"（生得）| "emergent"（創発）| "tool"（道具）
#   deviation_keywords  逸脱リスト全文検索用キーワード（空なら空タプル）
#   注意：この2つは**価値判断そのもの**（ユーザーの確定判断待ち）。
#     ⇒ 既存の NODES（2026-09-15時点で38件）は自動分類せず、全て None と () の
#       ままにしてある（＝宿題が見える状態）。
#     完全自動判定はユーザーが不採用と決めている（後から人が確定値を書き込む運用）。
N = None
NODES = [
    # ---- 感覚 ---------------------------------------------------------
    ...
    ("touch", "触覚", "touch", "sense", "逸脱",
     ...
     None, ()),
    ("touch_adaptation", "触覚の順応", "touch adaptation", "sense", None,
     "触覚の順応（末梢＋脳）を有効にする。有効にするには touch=true, "
     "somatosensory=true が要る（run/config.py の TARO_DEFAULTS、2026-08-13追加）",
     lambda c: bool(c.touch_adaptation),
     None, ()),
