...
#   左肘-53.74度＝狙いと逆の伸展側。報告：作業記録（非公開） 3節）。
#
...
class ToySupineEnv(SupineMimoEnv):
    ...
    def __init__(self, toy=None, toy_side="right", toy_offset=None,
                 ...
                 vergence=None,
                 # 【2026-09-13新設・設計_視覚を脳へ戻す 第2段】視覚と注意
                 #   （VisualAttention）。辞書（run/plugins/common/object_files.py の
                 #   `plugins.object_files` に書いていた設定と同じ形）を渡すと、
                 #   環境がVisualAttentionを持ち、毎step() `self._orienting.apply`の
                 #   直前で呼ぶ。既定None＝何も作らない＝1ビットも挙動が変わらない
                 #   （既存実験は`run/plugins/common/object_files.py`が後方互換で
                 #   従来どおり自分で作って呼ぶ）。
                 **kwargs):
        ...
        self._vergence = None
        # 【2026-09-13新設・視覚を脳へ戻す第2段】visual_attention=None（既定）なら
        #   何も作らない。実際の構築（モデルが要る各種反射と同じ位置）は下の
        #   self._use_vergence の構築ブロックの直後で行う。
        ...
        from run.world.parent import ParentLabeling, WordSchedule
        ...
        # 【段A・2026-09-13・撤去】ここで視覚と注意（VisualAttention）を組み立てて
        #   いた（2026-09-13の「第2段」）が、**誤りだったので撤去した**。
        #   理由(1) 環境の step の中で呼ぶと位置が変わり、発話が読む「注意している物」
        #     が前tick→今tickへずれて振る舞いが変わる（実測：注意.csv が step 1 から
        #     食い違う）。
        #   理由(2) 環境は世界の側なので太郎の語彙を覗けず lexicon=None を渡していた
        #     ＝語と物が結びつかない劣化版だった。
    ...
    def reset_model(self):
        ...
        #   再現・実測済み。別の担当が録画作成時に発見し実行時の属性上書きで
