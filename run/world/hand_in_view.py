...
各tickで、左右それぞれの手の中心が**左目・右目の両方のカメラ画角に入っているか**を幾何で判定する。
 ...
 ・既定は **片目でも**入っていれば「視野内」とする（mode="any"）。
   2026-07-20に「両目とも」から変えた（切り出し動画を目視して判断。関数自身の説明を見ること）。
   mode="both" にすれば両目必須に戻せる（環境変数 E_HV_MODE）
...
import run.world.toy_env as te  # noqa: E402
