...
def compute_status(*, env, actuation_model, actuation_mode, scene, taro_defaults,
    ...
    # ---- 1. 駆動モード＋対象関節数 ----------------------------------------
    # 【なぜ】この版の駆動ループは env.step(zero) のみ（run/viewer_tools/
    #   e_viewer_qt.py の main() を参照）。自発運動・脳・反射+共通駆動の
    #   いずれも配線されていない（駆動タブが未移植のため）。事実をそのまま書く。
    drive_line = "駆動: なし（ゼロ入力。駆動タブは次段階で追加予定）"
    ...
    # ---- 3・4. 触覚の順応／口元の報酬（既定値。実測はTaro構築後にしかできない）--
    # 【なぜ、仕様2-3節3・4】この版はTaro（脳）を構築していないため、実測値
    #   （touch_adaptationやmouth_touch_bonusの実際の計算結果）は存在しない。
    #   実体として読めるのは「このシーンでTaroを構築した場合に使われる既定値」
    #   （run.config.TARO_DEFAULTS）。値を書き写さず、辞書をそのつど参照する。
