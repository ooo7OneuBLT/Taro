...
注意：node が無い環境では「確かめられなかった」と出す。嘘の合格を出さない。
注意：これで分かるのは「エラーで止まらない」ことだけ。**見た目や操作感は人が見る**。
...
def main():
    ...
    for p in paths:
        ...
        if ops:
            ...
            #   注意：配線図の期待値（ドラッグ・ズーム）を全部の HTML に当てると、
            ...
            miss = need - set(ops)
            if miss:
                print(f"    注意足りない操作: {sorted(miss)}")
                bad += 1
        if res.startswith("注意"):
            bad += 1
    print("\n" + "-" * 78)
    print("  問題なし" if bad == 0 else f"  注意{bad} 件の問題")
    return 0
