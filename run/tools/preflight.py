...
def _説明文の腐りを知らせる():
    ...
    【2026-09-15・ユーザー指摘「あなたは過去の古い記録で判断している」】
      ...
      ここで見るのは2つ：
        ① 文書が古い     … コードが変わったのに作り直していない（バイト比較なので確実）
        ② 検査A          … 説明文の中の名前がコードに実在しない（消えた名前）
      どちらも**実験は止めない**（文書の遅れで走行を止めるのは本末転倒）。

      【検査Bを外した理由・2026-09-15】「説明文を書いた後にコードが変わった」も
      作ったが、**使い物にならなかったので preflight からは外した**。
        厳しく判定（モジュール冒頭の説明文だけ見る）→ 28本。うち約20本は
          その日に全文を読み直して直したファイルで、冒頭を触らなかっただけ＝空振り
        緩く判定（日本語の行を触れば「説明を直した」とみなす）→ 1本。
          実測すると `.py` を触ったコミットの **98%** が「説明を触った」になり、
          実質いつも黙る
      git の履歴が持っているのは日付と行番号だけで、**説明が正しいかは入っていない**。
      これはコミット時の確認（index_core --commit が説明文と変更を並べて止める）が
      引き受ける。`--stale` はおおまかな目安として手で叩けるように残してある。
    """
    try:
...
def _文献の散らばりを知らせる():
    ...
    【なぜ】2026-09-15 まで、文献調査は目標フォルダ下・doc/ など
