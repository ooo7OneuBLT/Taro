"""共有テーマファイルに、別システム側の言葉が混入していないかを調べる。

【なぜ要るか、2026-08-14】run\\viewer_tools\\e_viewer_qt_theme.py は
太郎本体Viewer（taro_core外だが太郎プロジェクト側のファイル）と、
別リポジトリのビューア(PySide6版, 別システム側)が共有するテーマ機構である。
依存の向きは 別プロジェクト -> Taro の一方向だけが許されており（太郎側が正、
別システム側が一方向で読みに行く）、太郎側のファイルに別システムの
都合（用語・パス）が紛れ込むと、この依存の向きが暗黙に逆転してしまう
（太郎を単体公開するときの事故にもなる。設計
作業記録（非公開）
7-2節）。
...
【何を見ないか（意図的に外している）】
    ...
    （仕様の依頼範囲。将来チェック対象を増やす場合はTARGET_FILESに追記する）。
...
# 別システムを指す言葉。太郎側のファイルに書いてはならない。
# 【なぜこの語のリストか】仕様に明示された例をそのまま使う。将来、別の
# organization固有語が見つかったらここへ追記する（本文が広く使う一般語
