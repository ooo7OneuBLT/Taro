...
TARO_DEFAULTS = {
    ...
    "lexicon_eta_push": (0.1, "連合器の対照学習：引き離しの学習率（mode='contrast'のみ"
                         "意味を持つ）", None),
    # 【2026-08-28新設・F2-13】周辺視も意味の判断に使う。
    #   人間は中心窩（細部）と周辺視（全体の配置）の両方で物を見分けるが、
    #   太郎は中心窩カメラ1枚だけをDINOv2へ渡していた（実測：板の見かけ49.8度に対し
    #   中心窩カメラは15度＝犬の首輪のあたりしか写らず、形でなく色で語を選んでいた。
    #   図＝F/logs/F2-12_視界確認/図_視界全体と中心窩.png）。
    #   True にすると語の意味を { "fovea": ..., "peripheral": ... } の2チャンネルで
    #   持つ（lexicon.py の channels）。**既定 False ＝ 従来と1ビットも変わらない**
    #   （周辺視の推論コストも払わない）。
    "lexicon_peripheral": (False, "周辺視（視野60度の眼球カメラ）も語の意味に使うか。"
                           "Trueで中心窩と周辺視の2チャンネルになる", None),
