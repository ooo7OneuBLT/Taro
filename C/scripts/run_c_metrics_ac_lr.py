...
def run(seed, n_train=3600, K=100, ckpt=600, n_eval=80):
    ...
    def closed_loop_probe(n=30, reach_len=4):
        """C4診断：閉ループ制御 vs 開ループ で、目標姿勢へどれだけ届くか。
        目標g＝過去に経験した姿勢(goal_buf)。同じ物理状態(MuJoCo save/restore)から：
          ・開ループ＝逆算した1行動を、リーチ全体(reach_len回ぶん)保持（撃ちっぱなし）
          ・閉ループ＝1回ごとに"実際に観測した状態"から再逆算して補正（にじり寄る、reach_len回）
          ・random／nothing(=何もしない, 開始姿勢のまま) を基準に、gへの到達距離を比較。
        【重要】補正の刻み k_inner ＝ K（順モデルの予測幅）に合わせる。第1版はK//8で予測幅
        ミスマッチ→閉ループが負けた。モデルが得意な予測幅で補正するのが正しい。"""
        mj = env.unwrapped
        ...
        for _ in range(n):
            ...
            o_open = const_rollout(a_open, reach_len * k_inner, qpos, qvel)
            o_closed = closed_rollout(g, z0, clp0, state["hidden"], state["prev_a"], qpos, qvel)
            o_rand = const_rollout(torch.empty(n_act).uniform_(-1.0, 1.0), reach_len * k_inner, qpos, qvel)
            do.append(mse(o_open, g).item()); dc.append(mse(o_closed, g).item())
            dr.append(mse(o_rand, g).item()); dn.append(mse(clp0, g).item())
            ...
            if term:
                reset_state()
        mc, mo, mr, mn = float(np.mean(dc)), float(np.mean(do)), float(np.mean(dr)), float(np.mean(dn))
        print(f"[CLOSEDLOOP seed{seed}] closed={mc:.4f} open={mo:.4f} random={mr:.4f} "
              f"nothing={mn:.4f} | closed/open={mc / max(mo, 1e-9):.2f} "
              f"closed/nothing={mc / max(mn, 1e-9):.2f} open/nothing={mo / max(mn, 1e-9):.2f}", flush=True)
