...
液の慣性でたわみ、クプラの弾性で戻る＝**高域通過フィルタ**として働く。
  Steinhausen のトルク振り子モデル（2次系）：
      ...
      T2 ≈ 4〜7秒       （長い方・粘性と弾性の比）
  注意：T1 が効くのは 16〜50Hz 以上で、太郎の物理は100Hz・頭部運動は数Hz以下なので
...
【時定数の値】
  ...
  注意：「新生児は時定数が成人の約半分」という記述は**一次典拠を特定できなかった**
...
#   成人  ：15〜25秒
# 注意：[Tier2] 「一次眼振の持続時間」と「フィルタの時定数」は厳密には同じ量ではない。
#   オーダーが合う値として採用する。
TIME_CONSTANT_NEWBORN = 10.0
TIME_CONSTANT_ADULT = 20.0
# 時定数が成人値に達する月齢。注意[Tier3・ARBITRARY] Weissman 1989 は生後2か月で
...
def time_constant(age_months=0.0):
    ...
    注意：線形補間は[Tier3・ARBITRARY]。向き（発達で延びる）だけが文献に一致
...
def read(vestibular_obs):
    ...
    注意：動特性を通していない生の角速度。VOR など**反射に使うときは
...
class SemicircularCanals:
    ...
    def update(self, omega, dt):
        ...
        注意：厳密な離散化（1次系のゼロ次ホールド解）を使うので、dt がどんな値でも発散しない。
        """
