...
【どう対処するか・3案あるうちの①を採用】
  ...
  ②MuJoCo が持つ頭の姿勢から重力方向を直接計算する
     ⚠️人間が持っていない情報を使う[ARBITRARY]。切り分け用の簡易版としてのみ可。
...
def read(vestibular_obs):
    ...
    ⚠️動特性を通していない生の比力。傾きを知りたいときは `OtolithOrgans` を使うこと。
    """
...
class OtolithOrgans:
    ...
    ⚠️入力は**頭部座標系**の比力であること（MIMo の vestibular_acc がそれに当たる）。
    ...
    def __init__(self, tau=GRAVITY_ESTIMATE_TAU, gain=OCR_GAIN, upright=None):
        ...
        # ⚠️仰向けの太郎では重力は体の背中方向から来るので、単純に -z とは限らない。
    ...
    def roll_angle(self):
        ...
        ⚠️ロールだけを取り出す（ピッチは実装しない）。基準方向 upright と
