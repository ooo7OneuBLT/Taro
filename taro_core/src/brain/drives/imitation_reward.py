# --- コピー元: B/src/taro/brain/instincts/imitation.py:1-39, 42-104, 107-124 ---
# --- 移植: 2026-08-22（F2「見た物の名前を言う」、設計：F/docs/設計_F2_初語
#     （見た物の名前を言う）.md 第2部「実装作業②」）。原本はB側に残す。
#     中身は無変更（コメントもB原本のまま）。
# --- 追加移植: 2026-08-23（12ヶ月産出実験の準備①。指示：文字ごとの信用割当を
#     産出に繋ぐ。B/src/taro/brain/instincts/imitation.py:42-104
#     compute_alignment_credit を移植。B原本は変更していない。中身は無変更。---
...
【F2での使い方】「親の発話」を「見た物から思い出した語」に、
（当初は lexicon.reverse_lookup が返す chunk を想定していたが、lexicon は
 2026-09-15に削除済み。いまの経路は 塊GRU と 言語海馬 の比較＝
 doc/脳の地図.md §0「語を選ぶ経路」）、「太郎の出力」を「generate()が実際に出した音」に差し替えるだけで、
このファイル自体はB原本と1文字も変えていない（設計「実装作業」表②）。
"""
...
def compute_alignment_credit(target_tokens, generated_tokens, vocab, vocal_tract):
    ...
    注意：逸脱：これは「人間模倣」ではなく既存AI研究側の工夫。乳児の語彙表現は
