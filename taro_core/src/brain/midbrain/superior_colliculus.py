...
注意：原論文は有料で本文を確認できていない。定数は複数の独立したモデリング論文
...
class CollicularMap:
    ...
    def __init__(self, width, height, fovy_deg,
        ...
        # 注意：「重みを掛ける」方式（旧）専用。格子方式では**使わない**（二重になる）。
        self.mag_area = (self.bu * self.bv) / (self.ecc + self.a) ** 2

        # 格子の細かさ。既定は計算コストとの兼ね合いで決めた（下記）。
        import os as _o
        # 2026-07-27：96x128 → **48x64**。実時間で動かすには 96x128・反復12 が重すぎた
        #   （1回24.7ms、制御周期10msの2.5倍）。48x64・反復8 なら 2.8ms。
        #
        # 【2026-09-15・訂正】ここにはもともと「48x64 まで落とすと環境で精度が落ちた
        #   （後半ずれ 0.154 → 0.490）。64x96・反復12 が折衷」と書いてあったが、
        #   **その 0.154 → 0.490 は存在しない差だった**。
        #   シードを固定せずに1回ずつ比べていたのが原因で、同一条件で3回まわすと
        #   0.319 / 0.215 / 0.189 とばらついていた（落とし穴チェックリスト 項59）。
        #   3シード平均で比べ直すと 96x128・反復12 → 0.333 / 48x64・反復8 → 0.350 で、
        #   設定間の差よりシードのばらつきのほうが大きい。
        #   コードは最初から 48x64 で動いており、**コードが正しくコメントが誤りだった**。
