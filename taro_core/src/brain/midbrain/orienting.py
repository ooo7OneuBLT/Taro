...
# 【2026-09-11・ユーザー指摘「意図的じゃないの消すんじゃだめなの？」で 0.05 → 0.30】
#   0.05秒は**この模型に反する後付けだった**。すぐ下の SACCADE_FRAC の注釈が
#   採用を宣言している Robinson 1975 の local feedback model は
#   「眼球位置の内部コピーと目標を比べ、**差がゼロになるまで**発射し続ける」。
#   時間で打ち切る規定はどこにも無い。
#   実測（F2-129・F2-133pre の毎ステップ記録、本番と同じ体）：
#     撃ってから 10ms=86度/秒 → 20ms=112 → 30ms=132 と**上がり続けたまま**
#     打ち切られていた＝頭打ち（筋力の限界）ではなく、時間で止められていた。
#     その結果、30%に縮めた後の目標が5度を超えると届かず、8〜20度の命令で
#     残った誤差の中央値が 5.4度だった。
#   ここは**安全網**として残す（届かない目標＝可動域の外などで永久に終わらないため）。
#   通常の動作では当たらない値にする。当たっていないことは1発ごとの記録
#   （長さが振幅に応じてばらつくか）で確かめる。
SACCADE_DURATION = float(_os.environ.get("E_SACC_DURATION", "0.30"))
...
#   （doc/文献調査/二語文/2026-09-11_サッケードのメインシーケンス_人間と乳児.md）。
#
...
#   判定してしまう**。既定 0.02 秒＝実測した遅れそのもの。
#   0 にすると 2026-09-11 以前の挙動（0.5度未満の命令が1ミリも動かない）に戻る。
#   詳細は下の `_on >= SACC_MIN_ON_S` の所のコメント。注意：[Tier3・ARBITRARY]
SACC_MIN_ON_S = float(_os.environ.get("E_SACC_MIN_ON", "0.02"))
...
#   （doc/文献調査/二語文/2026-09-11_サッケード後の保持_人間側.md）。名前のとおり
...
# 根拠調査：doc/文献調査/2026-08-21_IORの座標系とremapping.md
#
...
#   連射の上限発数（無限連射の保険）。
#   注意：「FRAC=0.3で4発＝残り24%まで詰まる」という計算を昔ここに書いていたが、
#     SACCADE_FRAC の既定は2026-09-12に 1.0 になったので、その計算はもう成り立たない
#     （1.0だと1発で残りが0になる）。USE_SACC_CHAIN の既定はOFFなので実害は無い。
...
#   `doc/文献調査/2026-08-19_乳児のIORと馴化.md`）、6ヶ月で機能しているとみてよい。
...
#   `doc/文献調査/2026-08-20_神経順応と勝者の疲労.md`で原文精読）が示す式：
...
#
# 【文献の答え】`doc/文献調査/2026-08-20_馴化のリセット条件.md`（3本とも
#   原文精読）：
...
class OrientingReflexV2:
    ...
    def set_map_target(self, h_dir, v_dir, t=None, latency_s=None):
        ...
        （`doc/文献調査/二語文/2026-09-10_注意と視線の回路_確定版_人間側.md`）。
    ...
    def apply(self, action, dt=None):
        ...
        if self.data is not None and len(self.angle_trace) < 60000:
            ...
            self.angle_trace.append((
                ...
                round(float(_hw[0]), 3),
                round(float(_hw[1]), 3)))
        # 【設計の由来・2026-07-27】1発の大きさを「位置の指令」にした。
        ...
        # 注意：これは2026-09-15まで三重引用符で書かれていたが、関数の最初の文では
        #   なかったため説明文として働いていなかった（ただの式）。コメントに直した。
        ...
        if self.data is not None:
            ...
            # 振幅で持続時間が変わる）
            #
            # 【2026-09-11・直し】この判定は**撃った最初のステップから**走っていた。
            #   筋の反応には20msの遅れがある（MIMo `mimoActuation/muscle.py:53`
            #   の tau=0.01・実測とも一致）ので、その時点ではまだ目は動いていない。
            #   結果、**目標との差が SACCADE_DONE_DEG(0.5度) より小さい命令は、
            #   撃った瞬間に「もう着いている」と判定されて終わっていた。**
            #   実測（今日の全走行17本・3333発）：
            #     0.5度未満の命令 154発 → **100%がまったく動かず・長さ 0.0 ms**
            #     1.0度以上       3179発 → 0%が動かず・長さ 40 ms
            #   （0.5〜1.0度の命令は1発も存在しない＝全部0.5度未満だった）
            #
            #   これが「1度未満の命令は向きの一致率が0%」の正体。
            #   向きが反対なのではなく**動いていない**（判定式が内積>0だったので
            #   内積ちょうど0＝不動を「不一致」と数えていた）。
            #
            #   人間に一律の不応帯は無い（1度未満も大きいサッケードと同じ運動の
            #   規則に従う切れ目のない分布）。確率的な不応帯はあるが、ずれ0.12度で
            #   70%不発・0.23度で50%（Poletti, Intoy & Rucci 2020【原文確認】）で、
            #   0.5度の一律カットは一桁大きく形も違う。
            #   調査：doc/文献調査/二語文/2026-09-11_小さいサッケードの扱い_人間側.md
            #
            #   直し方：**筋が反応する時間が経つまで終了判定をしない。**
            #   1発の途中で目標に届いたら終えるのは正しい（本来の役目）。
            #   撃つ前に終わらせるのが間違い。
            #   E_SACC_MIN_ON=0 で従来の挙動に戻せる（比較用）。
            _on = (SACCADE_DURATION - self._sacc_remaining)   # 撃ってからの経過[秒]
            if _on >= SACC_MIN_ON_S and max(abs(e) for e in errs) <= SACCADE_DONE_DEG:
                self._sacc_remaining = 0.0
            if self._sacc_remaining <= 0.0:
    ...
    def _should_hold(self):
        ...
        2. **人間は条件つきで保持していない。** 保持は脳幹の神経積分器が作り、
           **次のサッケード命令が来るまで、暗闇でも無条件に続く**
           （`doc/文献調査/二語文/2026-09-11_サッケード後の保持_人間側.md`。
    ...
    def _update_fatigue(self, step):
        ...
        モデル。`doc/文献調査/2026-08-20_神経順応と勝者の疲労.md`で原文精読）：
            τH・dH/dt = −H + kH・a・S
    ...
    # 【2026-09-15】ここに「ステップ1：複数フレーム比較による動き検出」という
    #   見出しだけが残っていた。中身は taro_core から外したときに消えている
    #   （update() の説明文を見ること）。見出しも消した。
