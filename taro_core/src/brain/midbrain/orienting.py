...
class OrientingReflexV2:
    ...
    def __init__(self, model, data=None, time_scales=TIME_SCALES, noise=LI_NOISE,
        ...
        self.sacc_log = []
        self._sacc_rec = None
        # 【2026-09-10・測定用】毎ステップの眼球の角度。撃っていない間の動きが
    ...
    def update(self, eye_image):
        """【2026-09-11・退避】自前の動き検出は taro_core から外した。何もしない。

        【なぜ外したか】この経路は「自分で画像から動きを見つけて、その方向へ撃つ」
        ためのもの。実測（F2-114pre、600歩）で **撃った回数 0 回**。同じ走行で
        地図を読む経路が 218 回すべてを撃っていた。使われていないだけでなく、
        **保持（サッケード後に眼球をその位置に留める力）の発動条件がこの経路に
        ぶら下がっていた**ため、保持が一度も働かない原因になっていた。

        【外した中身の置き場】`archive/2026-09-11_自前の動き検出.py`
        （動き検出・中心視野の重み・上丘格子の競合・重心。ビューアと過去の
        プローブが読めるように、コードはそのまま残してある）。

        【残っている属性】`strength` `h_dir` `v_dir` `motion_map` などは
        初期値のまま動かない。ビューアはこれを表示するので落ちない。

        【戻すとき】人間の上丘は自分でも目立ちを計算するので、この経路が
        人間模倣として不要になったわけではない。地図を1枚にまとめる設計が
        決まったら、地図の側へ統合して戻す（`F/docs/二語文/現状_見る側_2026-09-10.md`）。
        """
        return
    ...
    def _should_hold(self):
        ...
        【2026-09-11・条件を変えた】以前は「直近 HOLD_LOSE_SEC 以内に**自前の
        動き検出**が信号を出していれば保持する」という条件だった。これには
        2つの誤りがあった。

        1. **鍵が使われていないドアに付いていた。** 自前の動き検出は実測で
           撃った回数 0 回（F2-114pre）。地図を読む経路が 218 回すべてを
           撃っており、この条件は一度も満たされなかった＝**保持が一度も
           発動していなかった**（保持を切っても走行が1ビットも変わらないことで確認）。
        2. **人間は条件つきで保持していない。** 保持は脳幹の神経積分器が作り、
           **次のサッケード命令が来るまで、暗闇でも無条件に続く**
           （`F/docs/二語文/文献調査/2026-09-11_サッケード後の保持_人間側.md`。
           「面白い間だけ保つ」という条件つき保持を主張する一次資料は
           見つからなかった）。積分器が働かないと注視誘発性眼振＝中央へ
           流れ戻っては撃ち直す、を繰り返す。**太郎の症状がこれだった**
           （眼球が全ステップの100%で動き続け、速さ中央値136度/秒）。

        よって条件は「1発でも撃っていること」だけにする。次の命令が来れば
        そちらが目標を上書きするので、保持は自然に終わる。
        """
