...
関係なく各場所に値がある（`doc/文献調査/二語文/2026-09-10_注意は場所か物か_人間側.md`
で原文確認）。
...
class SalienceMap:
    ...
    Args:
        ...
        levels: 【未使用】保存されるだけで、どこからも読まれない。
            実際のピラミッドの深さは cs_pairs から自動で決まる
            （self._need = max(max(c,s) for c,s in cs_pairs) + 1。既定で9）。
            注意：この引数の既定値は7で、既定 cs_pairs が要る9に足りていない。
            もし読むように直すなら、まず既定値の食い違いから直すこと。
