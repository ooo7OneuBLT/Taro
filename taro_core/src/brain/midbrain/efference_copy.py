...
3. 行・列ともimg配列の素直な向き（+x=列が増える＝右、+y=行が増える＝下）で
   ...
   sign_v=+1.0（F2-107 JSONの既定と一致）。
   注意：ここにはもともと「走行中の自己検査（object_files.py の
   `_run_efference_self_check`）で反対と分かれば設定で反転する」と書いてあったが、
   **その関数はどこにも存在しない**（2026-09-15に確認）。符号は自動では直らない。
