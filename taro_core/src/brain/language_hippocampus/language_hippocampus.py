...
スロット。脳本体の分散した重みには混ぜない＝干渉しない）。
読み出しは行動に接続済み（2026-09-02・段階2）。recall_with_conf が返す
（語・似ている度・記憶の強さ）を塊GRUの自力候補と比べ、自信の高い方を言う
（brain_tick.py の _apply_word_production）。recall のほうは測定専用。
"""
