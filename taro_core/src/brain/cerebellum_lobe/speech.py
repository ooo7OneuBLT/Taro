...
# --- 元パス: B/src/taro/brain/cerebellum.py （F2-1で taro_core へ移植・2026-08-23） ---
# --- 設計: F/docs/設計_F2-1_喃語で口の内部モデルを作る.md 第4部 実装作業① ---

"""
発話小脳（Cerebellum） — 口の動きと音の対応を体験から学ぶ帳面（順モデル＋逆モデル）

【注意：手足の運動小脳（cerebellum_motor.py の MotorCerebellum）とは別物】
  このファイルは「口をこう動かしたらこの音が出た」という発話専用の対応表。
  taro_core/src/brain/cerebellum_motor.py は手足90関節の運動自動化を扱う、
  まったく別の実装（クラス名も別＝Cerebellum vs MotorCerebellum）。
  名前が紛らわしいため、ファイル冒頭にこの注意を残す。
...
class Cerebellum:
    ...
    def __init__(self):
        ...
        # 注意：定数：自動化の練習スケール（この回数でおよそ半分自動化）。未検証・後で
        # 感度確認と除去テスト（数を入れる原則）。
        # 【F2-1・決定4】今回（喃語フェーズ）はこの automatization() を呼ばない
        # （generate() 呼び出し側で cerebellum は渡すが automatization は
        #  generate() 内部が自発的に呼ぶ。未検証の決め打ちのため、まず入れずに
        #  動かす方針＝設計第2部決定4。値そのものは B からそのまま残す）。
