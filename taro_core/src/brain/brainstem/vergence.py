...
3. apply()の位置フィードバックのゲイン：設計の表に指定が無い。同じ眼球アクチュエータ群
   ...
   apply()は「左右の関節角の**和**(actual_left + actual_right)」を「目標のvergence_deg」に
   近づけるよう、左右へ符号を逆にして加算する（この式は共同運動の成分に影響しない。
   下記 _apply の実装参照）。
   注意：**和**である。眼球関節の回転軸の符号が左右で逆なので、輻輳・開散＝和、
   共同運動＝差になる（2026-08-28「重大な訂正」。それ以前は差と書いていた）。
...
class VergenceReflex:
    ...
    def apply(self, action):
        ...
        左目の指令 ＝ 共同運動の成分 ＋ 輻輳の成分÷2
        右目の指令 ＝ 共同運動の成分 － 輻輳の成分÷2
        （設計「1. 全体の構成」）。ここでは「左右の関節角の**和**
        (actual_left + actual_right) を vergence_deg に近づける」P制御として実装する。
        （回転軸の符号が左右で逆なので、輻輳は和。2026-08-28の訂正）
