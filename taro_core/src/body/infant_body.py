...
注意：文献で裏取りできたのは足長7.58cmのみ。下肢19.6cm・上肢20.96cmは測定定義が
...
#
# 注意：【2026-07-25 ラベルを [Tier1] から [Tier2〜3] に格下げ】
...
#   注意：値そのものは変えない（体型v3は身長・体重・頭身とセットで人間に一致させてあり、
...
#
# 注意：[Tier3] 薄め方（線形）に根拠は無い。太郎の既存の補正（首の筋力の回復曲線）が
...
# 注意：指のknuckle/middle/distalは連鎖するので、掌だけでなく指も含めないと
...
#
# 注意：【重要・2026-07-21 に踏んだ罠】**どの index が「長さ」かは部位で違う**。
...
_GROUPS = {
    ...
    "leg_thick":   (["geom:left_upper_leg1", "geom:left_lower_leg1",
                     "geom:left_lower_leg2"], 0),      # 注意脚長にも効く（bodyのposが参照）
    "arm":         (["left_uarm1", "left_larm"], 1),
    "arm_thick":   (["left_uarm1", "left_larm"], 0),   # 注意前腕→手の距離にも効く
...
def elongate_head(spec, ratio):
    ...
    **球のままでは頭囲・身長・見かけの3つを同時に満たす解が無い**。
    ...
    注意：目のカメラ位置は head の geom size から計算されるが、その計算は成長モジュール
...
def apply_head_mass(model, age=0.0, fraction=None, verbose=True):
    ...
    注意：慣性テンソルも同じ倍率でスケールする（形は変わらないので質量に比例する）。
    これを忘れると「重いのに回りやすい」物理的にありえない頭になる。

    注意：必ず首・四肢の筋力補正**より先に**呼ぶこと。首の補正は「頭を持ち上げられるか」を
    頭の質量から計算するので、後から頭を重くすると首の補正が古い前提のままになる。

    注意：**新生児期（age < HEAD_MASS_UNTIL_MO）だけに適用する**。四肢の筋力補正は
...
def apply_distal_mass(model, scale=1.0, verbose=True):
    ...
    **四肢の内訳**を測ると末端が成人比で極端に軽かった：
    ...
    注意：**正しい値は誰も知らない**：新生児の体節質量比は**実測が存在しない**
    ...
    **サイズは変えない**のが重要。`hand` 係数を振るとサイズも変わり、
    ...
    注意：慣性テンソルも同じ倍率でスケールする（`apply_head_mass` と同じ理由）。
    注意：手の指・足の指も含める（`left_hand` から先の子孫すべて）。
...
# 注意：`actuator_user` を後から変えても反映されない（初期化時に一度読むだけ）ので、
...
def scale_actuator_strength(model, aid, factor, actuation_model=None):
    ...
    注意：スカラーの fmax は**全身に一律**なので、その場合は何もしない（関節ごとに
...
# 注意：MuscleModel は jnt_stiffness=0 で、筋の受動力 fp(lce) で代替している。
...
# バネの強さ。注意新生児の関節の受動剛性(N·m/rad)の文献値は**存在しない**（調査済み）。
...
#       注意：ただし対象月齢も測り方も同じとは保証できないので「オーダーが合う」までの主張。
FLEXION_STIFFNESS = 2.0
# 生理的屈曲は発達で解消する（膝：出生21.4度→3ヶ月10.7度→6ヶ月3.3度）。
# 注意：この月齢まで一定に保ち、それ以降は触らない[Tier3・簡略化]。
...
# 注意：[Tier2・近似] バネは伸張反射と**機能は似るが機構は違う**：
...
#              注意：この値は **qpos0 の誤りを修正した後**に作り直したもの。
...
#   注意：これは**筋を含まない骨・靭帯だけ**の値＝生体の下限。実際はこれより硬い。
#   → 上限側の 0.40 を採用する [Tier2]。
#   注意：MuJoCo の jnt_stiffness は方向で変えられないので、
...
#
# 【-25度はどこから来たか】注意[Tier3・ARBITRARY]
...
#
# 注意：[Tier3] 18ヶ月まで一定に保つのは簡略化。本当は体が大きくなれば剛性も上がるはずだが、
...
def apply_physiological_flexion(model, age=0.0, stiffness=None, verbose=True,
    ...
    **これ以上は伸展できないという限界**であって、「ここに戻る」という目標角ではない。
    ...
    if mode == "spring":
        # 注意：旧実装。曲がった関節を伸ばしてしまうので既定では使わない（上記参照）。
        ...
        if verbose:
            print(f"[flexion/spring] age={age}mo: {n} joints, stiffness={k} "
                  f"({' '.join(applied)}) 注意旧方式（曲がった関節を伸ばす）")
    ...
    for base, target in FLEXION_TARGETS.items():
        ...
        for side in ("right_", "left_"):
            ...
            # 注意：【2026-07-26 修正】ここで model.qpos0 を書き換えてはいけない。
...
#
# 注意：新生児の一過性斜視をあとで入れるなら、ここに「時々ずれる」を足す形になる。
#   今は「基準は正中位」だけを実装する。
...
# 対象が視野の上端に来てしまう。
#
# 注意：[Tier3] **人間の基準は正中位**（下の説明を参照）。ここを0以外にするのは
...
#   値には議論がある。詳細: doc/文献調査/座位保持/2026-08-17_眼球の受動的中央復帰.md
...
def apply_neck_tone(model, age=0.0, stiffness=None, target=None, joints=None,
    ...
    注意：これは**工学的判断（安定性優先）であって実測値の再現ではない** [Tier3]。
