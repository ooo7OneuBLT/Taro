...
【注意恣意的な部分（逸脱リストに記録すること）】
  ①~~目標比を1.0に置いた~~ → **2026-07-28 に 0.5 へ改訂し、根拠が付いた**。
    ...
    注意：ただし姿勢が違う（文献は腹臥位懸垂／太郎は仰向け）ので測り直しが要る。
...
HEAD_CONTROL_AGE = 4.0         # 首がすわる月齢 注意曲線の形は恣意的（上記②）
...
#   注意：**1.5 の 1.5 に根拠は無い**。根拠があるのは「4ヶ月で1.0を横切る」ことだけ。
...
def head_gravity_torque(model, data):
    ...
    注意：**呼ぶ前に mj_forward が済んでいること**。data.xanchor / data.xipos は
...
def target_ratio_for_age(age, birth_ratio=TARGET_RATIO_AT_BIRTH,
    ...
    注意：線形補間は恣意的[Tier3]。ただし「新生児が最も弱く成長で強くなる」という**向き**は
...
def apply_newborn_neck(model, data, age, birth_ratio=TARGET_RATIO_AT_BIRTH,
    ...
    **仰向けにした姿勢が反映されていない状態**で頭の重力モーメントを測っていた。
    ...
    注意：mj_forward は qpos0（リセット時の姿勢）での物理量を計算するだけで、
      settle（体が床に落ち着くまでの100ステップ）は走らない。完全一致はしないが、
      「腕が0.76cm」のような桁違いのズレは消える。

    注意：**恣意的な逸脱**。ROM・swivel・tilt_side・他の関節は一切変更しない。
    ...
    #   注意：直接 actuator_gear を書くと MuscleModel では次のステップで消える。
    ...
    if verbose:
        note = "" if ok else " 注意[fmaxがスカラー＝関節ごとに変えられないため未適用]"
