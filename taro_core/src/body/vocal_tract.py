...
これにより「ま」と「ぱ」が少ないパラメータ違いで表せるという
身体的な近さが脳の中に自然に生まれる。
...
_ARTICULATION_TABLE = {
    ...
    # な行（歯茎・鼻音・有声）※に=硬口蓋
    # 「な」と「ん」を区別するため、「な」は歯茎(2)で鼻音
    # 注意：「ん」を軟口蓋(5)にしていたのは**旧定義**。B-6で両唇(1)に変えた
    #   （軟口蓋はstage3が要り、早期に産出できなかったため。下の定義を見ること）
...
# 【⚠2026-08-27】この座標 (0,0,1,0) は上の表で「あ」に割り当て済み＝
#   speak() は必ず「あ」を返し、太郎の口から「ー」は原理的に出せない。
#   長音は口の形(4パラメータ)ではなく時間の現象なので、この表では表せない。
#   → 耳の入口 taro_core/src/senses/hearing.py の expand_long_vowel() が
#     「ー」を直前の母音に展開するので、通常この行は使われない
#     （残しているのは hear() に直接「ー」を渡す古い呼び出しへの保険）。
