...
【探索反射の実装形式】
 ...
 注意：ゲイン ROOT_GAIN は恣意的（速度でなくトルク割合。接触方向へ向く十分な大きさに置く）。
 ...
 **顔前面・目より下・中央** の領域を「口周辺(perioral)」とする（可視化で確認済み：
 ...
 注意：唇だけを分離する解剖学的マーカーはMIMoに無いので、頬〜口の広い perioral 野になる。
...
ROOT_GAIN = 8.0            # 探索反射：口中心からのずれ(m)→首トルク割合。注意恣意的
SUCK_DISCOMFORT_DROP = 0.02   # 吸啜1秒あたりの discomfort 低下量。注意恣意的（迷走神経鎮静の代理）
...
class OralSystem:
    ...
    def __init__(self, model, data, touch):
        ...
        self.touch = touch
        self.model = model
        # 注意：2026-07-21修正：内臓(discomfort)は100物理stepに1回しか進まない
    ...
    def rooting_override(self, action):
        ...
        注意：符号（どちらのトルクで顔がどちらを向くか）は実測で合わせる前提。
        """
